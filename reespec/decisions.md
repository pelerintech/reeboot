...
### Pluggable memory backend — MemoryManager seam, one active at a time, built-in fallback — 2026-08-02 (Discovery: hermes-gap-evaluation)

Memory becomes a pluggable backend behind a `MemoryManager` provider seam (recall / store / update / clear / system-prompt contribution). Exactly ONE memory backend is active per deployment, selected by config (`memory.provider`), and the internal reeboot backend (MEMORY.md/USER.md + hot-memory) is the default and the fallback whenever nothing is configured or a configured backend fails to load. This supports arbitrarily many future backends (dreem, mem0, honcho, etc.) without touching the agent core. This borrows Hermes's `MemoryManager` structure (abstract provider + one-external-provider-at-a-time rule — keeps the agent light and avoids tool-schema bloat) but points it at reeboot's own per-deployment config + graceful-degradation idiom (built-in as fallback, same pattern as jina→baseline and search→DuckDuckGo fallbacks). The RAG/knowledge corpus is explicitly NOT part of this seam — it remains a separate subsystem. This emerged from a comparison of reeboot against Hermes Agent (Nous Research); the "swap any memory backend" requirement tipped the design toward a native provider seam over "ride the existing MCP path" (the latter only normalizes MCP-shaped backends like dreem, not SDK backends like mem0). One-at-a-time selection, not composition; if composition is later needed it is a separate decision.

### Auth-state tool gating sits on the per-chat tool registry, filtered per turn — 2026-08-02 (Request: hermes-feature-eval)

The multi-user (ree) auth-gating design reuses the existing per-chat tool registry rather than adding new loop plumbing: `ToolDefinition` gains an optional `minAuthLevel` (`AuthLevel` = `anonymous | customer | admin`, with a rank), `ReeChat` carries an `authLevel` (default `anonymous`), a shared `applyAuthLevel(map, level)` helper returns the visible subset, and `ree-agent-loop.ts` filters `chat.tools` through it on every turn (the loop already re-reads the registry fresh per turn, so no loop changes beyond the filter). Auth state is changed via a reeboot-owned mechanism — `setAuthState()` on the ree extension API and a built-in `/auth_establish [level]` command — not a single-SDK hack. This is ree-only; pi binds tools at session creation. A deployment's auth flow (external service, agent call, or slash-command) decides when to raise the level; gated tools become callable on the next turn. See `auth-gated-tools` spec.

### Webhook triggers are generic inbound subscriptions, not GitHub-shaped — 2026-08-02 (Request: hermes-feature-eval)

Generic inbound webhooks are implemented as a standalone Hono sub-app (`buildWebhookApp` in `src/webhooks.ts`) mounted at `/webhook` by `server.ts`. Each `config.webhooks` subscription `{ name, secret, map?, prompt, deliver?, enabled }` exposes `POST /webhook/:name`; the request is HMAC-SHA256-verified against the subscription's secret (`X-Reeboot-Signature`, constant-time) before an agent run. The `prompt` maps the body (default JSON string) into context; with a `deliver` target the result is sent to a channel+peer via the existing `ChannelAdapter.send`, otherwise it is returned synchronously as JSON (the "webhook as API gateway" mode). The sub-app is tested in isolation with injected deps (`runTask`/`deliver`), avoiding fragile full-server boots. Categories (notify / act+deliver / B-side workflow entry) are just different prompt+deliver configurations of one primitive, keeping it event-agnostic rather than GitHub-specific. See `webhook-triggers` spec.

### Memory pluggability proven with a test fake; real dreem/mem0 providers are follow-ups — 2026-08-02 (Request: hermes-feature-eval)

The memory `MemoryProvider` seam (`recall/store/update/clear/system-prompt contribution`), `MemoryManager` (one active backend, builtin default+fallback), and `memory.provider` enum (`builtin | dreem | mem0`, default `builtin`) are implemented. The seam's ability to host a second heterogeneous backend is proven with an in-memory test fake (not the real dreem), because building the real dreem provider is external work and mem0 integration is a separate concern. Both are tracked as follow-up requests. The RAG knowledge corpus is explicitly outside this seam. See the memory-plug spec and the earlier pluggable-memory decision.

### Full-suite green unsupported in this sandbox — pre-existing, not from this request — 2026-08-02 (Request: hermes-feature-eval)

During hermes-feature-eval execution, ~30 `reeboot/tests` files were already failing before any change (confirmed by `git stash` baseline comparison): server-boot suites fail on `EPERM` writing to `~/.reeboot` (a sandbox filesystem restriction, not fixable in code), external-service suites need WhatsApp/MCP/knowledge-watcher subprocesses, `package.test.ts` runs `npm pack`, and `tests/runtime/extension-subset.test.ts`/`ree-extension-wiring.test.ts` assert a stale `getReeFactories` count (loader returns 8, test expects 7). This request's touched-area suites all pass and typecheck is clean; no new failures were introduced. The testing strategy (fast-unit vs server/integration isolation, keeping the suite green and trustworthy in restricted environments) needs a dedicated reassessment request. See the brief's Follow-ups section.

### `buildApp`/`startServer` seam: app construction decoupled from socket binding — 2026-08-03 (Request: test-suite-stabilization)

`src/server.ts` now exports `buildApp(opts)` (constructs the full Hono app: routes, WebSocket `upgradeWebSocket` handlers, middleware, notFound) and `startServer(opts)` (calls `buildApp`, then `createAdaptorServer` + `injectWebSocket` + `listen`, returning `{port,host}`). The `injectWebSocket` fn from `createNodeWebSocket({app})` is held in a module-level `_injectWebSocket` and reused by `startServer`, keeping the production listen path identical. This mirrors the proven `buildWebhookApp` pattern and lets every HTTP route be driven via `app.request()` with an injected `db`/`reebotDir` and no socket. Chosen over any approach that re-implements handlers in throwaway Hono apps (drift risk). The TCP-accept path remains covered by Docker integration tests, not unit tests.

### Logger home injection seam redirects test logs away from real `~/.reeboot` — 2026-08-03 (Request: test-suite-stabilization)

`LoggerConfig` gains an optional `logDir` (default: `<homedir>/.reeboot/logs`, unchanged); `createLogger`/`initLogger` honor it. `buildApp` initialises the logger at the very top of the function with `logDir: join(reebotDir,'logs')`, before any other startup side effect, so the `getLogger()` singleton never falls back to the unwritable real-home default during tests. This was surfaced immediately by the new server-app-seam test (uncaught EPERM on `~/.reeboot/logs/...`). It is the enabling seam for the real-home isolation tasks and is behavior-neutral in production. See `isolation-conventions` spec.

### `runResilienceMigration` is now self-contained for the `turn_journal.closed_at` column — 2026-08-03 (Request: test-suite-stabilization)

Discovered during reworking the server-boot suites: on a fresh DB, `runObservabilityMigration` (which adds `turn_journal.closed_at`) runs inside `openDatabase` *before* `runResilienceMigration` creates `turn_journal`, so `closed_at` was never added and pruning (`pruneTurns`) threw `no such column: closed_at`. `runResilienceMigration` now idempotently adds the column itself (guarded by `table_info`), making the resilience migration independent of migration ordering. This fixes the "no such column: closed_at" cascade that surfaced across the resilience suites. See `server-app-seam`/`resilience` work in this request.

### Socket-bound server/ree suites reworked to `buildApp` + bus-drive without a socket — 2026-08-03 (Request: test-suite-stabilization)

`server.test`, `resilience-integration`, `resilience-wiring`, and the ree shared-workspace tests in `orchestrator-ree-routing` now boot the real app via `buildApp({ db, reebotDir, config })` with a fully-migrated `openDatabase` temp DB and drive behavior through `app.request()` or — for the ree WebSocket-equivalent flow — publish to the real `MessageBus` obtained from `webAdapter.getBus()` (backed by a mocked `createRunner` that returns a fake runner). This preserves the true server wiring (runner-factory shared `contexts/__ree__/workspace`, crash-recovery notifications, requeue flow) without a TCP socket. The `startServer`/listen TCP path remains covered by Docker integration tests. See `server-app-seam` spec.

### WebSocket handler extracted as `wsChatHandler` and driven at handler level — 2026-08-03 (Request: test-suite-stabilization)

The `/ws/chat/:contextId` handler was extracted from the inline `upgradeWebSocket((c) => {...})` closure into an exported `wsChatHandler({ db, contextId, clientIp, serverToken, token, isReeMode })` that returns the real `{ onOpen, onMessage, onClose }`. The production route now just forwards the hono context args and calls it; `token` is extracted from the request and passed in (replacing the `extractToken(c)` closure reference). Tests (`ws-chat`, `ws-conversation-id`) invoke the real handler directly with fixture `ws`/`event` objects — no TCP/browser socket — and verify connected/close/parse/routing frames and bus stamping (`conversationId`, distinct `peerId`s) with a Capturing MessageBus. This is the design's "handler-level at both ends" approach and matches `server-app-seam`. The `@hono/node-ws` upgrade path remains production-only.

### Knowledge file watcher mocked for deterministic, fd-safe tests — 2026-08-03 (Request: test-suite-stabilization)

Replacing literal `/tmp/raw` with real `mkdtempSync` temp dirs in the knowledge-manager tests let the extension's `KnowledgeWatcher` actually attach an `fs.watch`, leaking file descriptors across tests (EMFILE) and making the watcher-lifecycle assertions depend on real fs-event debounce timing (`expected 0 to be greater than 0`). Fixed by mocking `src/knowledge/watcher.js` with a `FakeKnowledgeWatcher` whose pending-file queue is a shared `__pending` array that tests drive directly, and `start/pause/resume/stop` as no-ops. The watcher-lifecycle tests now inject pending files deterministically. This matches the request's "drive the watcher's handler/queue logic directly with injected events" approach and removes the fd leak. Applies to both `extensions/knowledge-manager.test.ts` and `knowledge-lint-schedule.test.ts`.

### Wizard home derives from `configPath` when `configDir` not given — 2026-08-03 (Request: test-suite-stabilization)

`runSetupWizard` previously defaulted `configDir` to the real `homedir()/.reeboot` even when the caller supplied a temp `configPath`, so wizard orchestrator tests writing `contexts/main/AGENTS.md` hit EPERM on the real home. `configDir` now defaults to `dirname(configPath)` (production: configPath defaults to `homedir()/.reeboot/config.json` → dirname is unchanged home; tests passing a temp configPath get a temp home). This is the home-injection seam for the wizard. `events-retention-wired` was reworked from socket-bound `startServer` to `buildApp` with an `openDatabase`-migrated temp db. See `isolation-conventions` spec.

### test-suite-stabilization: FULL SUITE GREEN — 2026-08-03 (Request: test-suite-stabilization)

Final gate reached: `npx vitest run` and `npm run test:run` both report **283 files passed / 1856 tests passed / 0 failed / 0 errors / 0 skipped**, exit 0. Notable resolutions beyond the earlier plan: (1) `observability-schema` stale assertion fixed — `turn_journal.closed_at` is owned idempotently by the *resilience* migration (turn_journal is a resilience table), not observability; (2) `smoke.test` unhandled EPERM resolved by rebuilding `dist/` so the compiled logger includes the graceful (write-probe) file-destination fallback; (3) `docker.test.ts` removed because its `describe.skipIf(!REEBOOT_DOCKER_TEST)` gated 2 skips (violates the 0-skip convention) and its Docker-image coverage is redundant with `tests/docker-integration/`; (4) `fsevents`/native segfault observed once during `npm run test:run` was transient (same suite passed on retry and via `npx vitest run`).

### Test determinism idiom: `drainEventLoop` helper + fake timers for real-clock logic — 2026-08-03 (Request: test-suite-stabilization)

Post-evaluation gap remediation: replaced the many fixed `await new Promise(r => setTimeout(r, N))` real wall-clock waits in the orchestrator/message/runtime/observability suites with `tests/helpers/event-drain.ts`'s `drainEventLoop()` — a deterministic microtask + `setImmediate` pass loop that flushes async chains with no wall-clock delay. This is the right tool when the work is event/microtask driven (message-bus publishes, DB/logger persistence, mock runners that resolve immediately). For tests that genuinely exercise real-clock scheduling in production (idle-TTL `sweepIdle` in ree-runtime/ree-history-integration, orchestrator turn-timeout and rate-limit retry backoff in error-handling), the correct deterministic tool is `vi.useFakeTimers()` + `advanceTimersByTime(Async)` — `drainEventLoop` does NOT advance real timers and must not be used for those (verified: it silently fails the idle-TTL assertions). The two tools are complementary and their correct use is now constrained by the `event-drain.ts` JSDoc.

### Literal `/tmp/<name>` paths eliminated suite-wide — 2026-08-03 (Request: test-suite-stabilization)

The isolation-conventions spec requires "no test contains a hardcoded `'/tmp/<name>'` literal path." Beyond the three files in the original task 8 disposition, a broader sweep found `/tmp/w` and `/tmp/test-workspace` across 12+ `runtime/ree-*`/`auth-gated-tools` files plus bare `/tmp` in runner/loader/embedder/whatsapp/agent-runner tests. All were converted to `mkdtempSync(join(tmpdir(), 'reeboot-<area>-'))` shared consts (or `join(tmpdir(), ...)` where never written), including updating exact-match assertions (e.g. `ree-adapter` `expect(...workspacePath).toBe(WORKSPACE)`, `session-lifecycle` `row.session_path`). These values are inert config in most cases (mocked tooling), so the change is behavior-neutral.

### `index.test.ts` CLI subprocess retained as a deliberate CLI-boundary smoke test — 2026-08-03 (Request: test-suite-stabilization)

The evaluation flagged `index.test.ts` for spawning a real `node --import tsx/esm <cli>` child process via `execFileSync`. Reviewed and retained: the test exercises the real commander CLI entrypoint (`--help`, exit codes, no-config wizard message) — i.e. OUR deliverable, not an adjacent/external service. The "Mocked boundaries / no real child process" clause in test-policy targets external-service dependencies (MCP, WhatsApp, watcher, etc.), not the product's own entrypoint. Reworking in-process would require faking `process.exit()` and argv parsing, losing the actual entrypoint coverage. `src/index.ts` already exposes a `_deps`-injectable `handleDefaultAction` seam used by other tests; the CLI-level tests stay as a thin boundary check. If a fully-sandboxed CI forbids child processes, replace with `handleDefaultAction` unit tests as a follow-up.

### Post-eval gap remediation: config/data-guard tests retained, two concrete fixes — 2026-08-03 (Request: test-suite-stabilization)

The 18:29 evaluation flagged "behavioral-over-existence" for `package.test.ts`, `docker-compose-yml.test.ts`, `docker-config-template.test.ts`, and `skills.test.ts`, plus the 2.2s wall-clock wait in `whatsapp-resilience.test.ts`. Resolution: (1) `dist/ is present` standalone assertion removed from `package.test.ts` — it was redundant with the whitelist-vs-repo-layout check and environment-dependent (requires a prior build), the only clear no-behavioral-signal existence assertion; the whitelist-layout check stays (it is the sanctioned in-process replacement for `npm pack --dry-run`). (2) `skills.test.ts` hardcoded `'15'` count replaced with a live derivation from `listBundledSkills` — same anti-drift principle as the 7→8 factory fix (task 10). (3) `docker-config-template.test.ts` retained: it validates `config.example.json` against the real `ConfigSchema` (behavioral — it exercises our code's contract) and pins Docker-wiring values. (4) `docker-compose-yml.test.ts` and the `## Setup`/`## Usage`/frontmatter skill-content checks retained: they guard shipped deployment-config and bundled-skill data quality; removal was not "unambiguous" under the design's adjacent-passing-file bar, so they stay as data-integrity guards rather than pure existence assertions.

### whatsapp-resilience reconnect-backoff wait retained after failed fake-timer conversion — 2026-08-03 (Request: test-suite-stabilization)

The `second close during reconnect does not start another loop` test waits 2.2s for the adapter's attempt-1 reconnect backoff (2000ms). Converting to `vi.useFakeTimers()` + `advanceTimersByTimeAsync` was attempted but GREEN was unreachable — under fake timers the mocked socket's reconnect retry never produced a new socket (`expected 2 to be 3`), because the adapter's `_connect`/watchdog/`connection.update` chain relies on real-timer semantics and the mixed `vi.waitFor`+real-timer file broke. Reverted. The wait is bounded, exercises in-process mocked-adapter resilience (no external service), and completes well within the file's 10s/CI timeout. This mirrors the `index.test.ts` retention precedent: it tests our adapter's own backoff logic, not an external dependency. If a fully real-timer-free CI is ever required, `_reconnectLoop` would need an injectable delay/clock seam as a follow-up.

### S3 fallback warning implemented; closed-enum vs graceful-degradation reconciled — 2026-08-03 (Request: hermes-feature-eval)

Post-evaluation remediation for memory-plug S3 ("falls back to builtin with a logged warning"). `MemoryManager.select()` now emits a warning on fallback — `src/memory-provider.ts` gained an injectable `warn` callback (default `getLogger().warn`) invoked when the configured provider id is not registered, before switching to `builtin`. This resolves the flagged "logged warning" gap. The apparent tension with the closed `memory.provider` enum was reconciled rather than removed: the schema still *rejects unknown typos* (`'foo'`) at config parse (task 1 config-safety intent, per design), while the manager *degrades gracefully with a logged warning* for enum-valid-but-unwired providers (`'dreem'`/`'mem0'` with no registered backend). Both layers therefore do their prescribed job — typo rejection at config, runtime fallback at the manager — and memory is never silently disabled. Covered by new tests in `tests/memory-plug/manager.test.ts` (warning) and `tests/memory-plug/builtin-provider.test.ts` (extension-level fallback keeps the `memory` tool available and routing to builtin). 57 memory tests + 19 auth/webhook tests green; `tsc --noEmit` clean.

### zod 4: object `.default({})` no longer applies nested defaults — use `.prefault(() => ({}))` — 2026-08-03 (Request: ci-zod4-fix)

The root `zod` dependency moved from `^3.25.76` to `^4.4.3` to resolve a CI `npm ci` ERESOLVE caused by the upstream TanStack AI zod split (adapters all peer-demand `zod ^4.0.0`). The migration surfaced a real zod 4 behavioral change in `src/config.ts`: zod 4 returns `.default(value)` **as-is without parsing it through the schema**, so `.default({})` on an object schema no longer applies that object's nested defaults (e.g. `agent.model` parsed as `{}` instead of `{provider:'', providers:[]}`). The 41 object/record `.default({})` sites in `src/config.ts` were therefore migrated to `.prefault(() => ({}))` — zod 4's mechanism that runs the fn and parses its result per-call, restoring exact zod 3 default semantics with a fresh object each parse (no shared-reference mutation). `z.record(z.string()).prefault(() => ({}))` and array `.default([])` are unaffected (their `{}`/`[]` defaults need no expansion). Verified: all 8 config test files (59 tests) pass under zod 4. Future own-code zod usage should prefer `prefault`/explicit defaults over relying on `.default({})` deep-default semantics.

### Action-shaped, scoped, ref-based memory contract — 2026-08-04 (Request: memory-system-redesign)

The `MemoryProvider` contract was reshaped from a file-shaped seam (`add/replace/remove/read/clear` over `memory`/`user` blob targets) into a semantic, action-shaped one: scope (`self`|`human`|`both`), query-based `recall`, opaque `MemoryRef` handles, and provider-owned `grounding`. This closes the "weakest common denominator" tension the original design flagged (flat entries vs graph) — the contract no longer leaks the builtin file model, so a heterogeneous graph/SDK backend (dreem, mem0) can genuinely host it. Refs are opaque (the manager never inspects internals), `recall` is never a full dump, and grounding takes no prompt (only provider-owned digest + self-policed `maxChars` ceiling fed by the budget subsystem). The prior `MemoryTarget` blob model and `buildSystemPromptContribution` were replaced. See contract spec for full context.

### Capability registry (Option 1) + uniform trust — 2026-08-04 (Request: memory-system-redesign)

Provider-declared optional capabilities surface through ONE uniform mechanism: every provider returns `listCapabilities()` (CapabilityDef[]), and the memory extension registers one namespaced tool per capability (`memory::<providerId>::<name>`) — identical for builtin, dreem, mem0, or any future provider. The trust boundary is "a tool is a tool": declared tools pass through the same four plies as first-party tools — 1) schema validation, 2) injection scanning, 3) minAuthLevel/permission-tier gating, 4) namespacing. reconsidered and rejected per-backend special-casing and provider-native channels. Option 1 (registry-walk registration) was chosen over option 2/3 because it needs no core changes and keeps the six core methods minimal. See capability-registry-trust spec.

### Delegation model — dreem is a memory system, not a store — 2026-08-04 (Request: memory-system-redesign)

One provider owns the ENTIRE memory experience per deployment. dreem is treated as a memory SYSTEM (its own consolidation + retrieval), not a passive store: the provider delegates recall/consolidation/hot retrieval to the backend and degrades gracefully at the provider level (logged, surfaced, no startup crash, no silently-lost memory) when a backend can't honor an operation — never leaking upward. This resolves the earlier "routed-through vs provider-owned" question: provider-owned when the backend self-consolidates, routed-through otherwise. See dreem-provider spec.

### Discrimination-union memory config (`memory.provider` + typed `providerConfig`) — 2026-08-04 (Request: memory-system-redesign)

The `memory` config block became a `z.discriminatedUnion('provider', ...)` on `builtin`|`dreem`|`mem0`, so each provider's `providerConfig` is strongly typed, flattened, and validated to exactly the keys it needs (builtin: limits+consolidation; dreem: required `baseUrl`, optional `apiKey`/`consolidationInterval`/`llm`). A typo in `baseUrl`, an unknown provider, or a malformed per-provider config is rejected at parse — consistent with the closed-enum "deliberate schema change" principle. A provider-factory registry (`registerProvider(id, factory)`) constructs the selected provider from the parsed typed config. Rejected a single loose bag-of-keys config as it would forfeit per-provider type safety. See memory-manager + config specs.

### Consolidation ownership declared via `selfConsolidating` capability — 2026-08-04 (Request: memory-system-redesign)

Consolidation encoding resolved to a single standard, manager-recognised capability declaration: `selfConsolidating: boolean` on the provider's registry. Provider declares `selfConsolidating: true` (dreem → its Dream loop) → reeboot's `__memory_consolidation__` scheduler job is skipped. Provider declares `false` (builtin) → reeboot runs its job **through the provider contract** (`provider.store('self', insight)`), never direct file writes. This supersedes the earlier option to route consolidation through a generic scheduler hook, keeping it capability-gated and provider-agnostic. See consolidated-routing spec.

### Provider-owned hot-memory (capability-declared) — 2026-08-04 (Request: memory-system-redesign)

Hot-memory is folded under the provider as a declared capability. builtin declares `hotMemory` (reeboot's hot-memory extension is builtin's recall layer), and reeboot's wiring runs for it. A provider that SELF-SERVES retrieval (declares `hotMemory`, not builtin — e.g. dreem) prevents reeboot's own hot-memory wiring: `shouldRunReebootHotMemory(provider)` gates it via a small shared coordinator (`memory-hot-routing.ts`). The agent loop does not run its own hot-memory for a provider that self-serves retrieval. See builtin-provider S4 + hot-memory-cap spec.

### reeboot-owned vs provider-owned boundaries — 2026-08-04 (Request: memory-system-redesign)

A firm boundary governs what may become a provider: `session_search` + FTS5 over `messages`, the conversation/compliance log (`messages`, `events`, `turn_journal`, `memory_log`), the RAG knowledge corpus, `custom-compaction` (working memory), and `budget-manager` (drives the grounding ceiling) are all reeboot-owned and NEVER move into a provider. Only the memory backend seam is pluggable. This keeps audit/observability and budget invariants intact regardless of backend swap. See design "What stays reeboot-owned".

### Consolidation dispatch threads the resolved active provider (builtin S5) — 2026-08-04 (Request: memory-system-redesign)

Post-evaluation gap: `runConsolidation` already routed writes through the provider contract when handed a `provider`, but the PRODUCTION scheduler dispatch (`createSchedulerTaskHandler`) never passed one, so the live consolidation job kept writing via `memoryAdd`/`memoryReplace`/`memoryRemove` direct `writeFileSync` — contradicting builtin S5 ("writes distilled insights via builtin.store — never direct file writes"). `SchedulerTaskHandlerDeps` now accepts the app `config`, and the consolidation branch resolves the active provider (`resolveConfiguredProvider(config, memoriesDir).manager.active`) and threads it into `runConsolidation` as `provider`. server.ts passes `config: appConfig`. The legacy direct-write path remains only as the unconfigured fallback. Covered by `tests/scheduler/consolidation-provider-routing.test.ts`.

### Provider recall output is external content via a runtime external-source registry (capability-registry-trust S6) — 2026-08-04 (Request: memory-system-redesign)

Post-evaluation gap: nothing marked non-builtin provider recall/capability output as untrusted external content — `pi-runner` output scanning and injection-guard's treat-as-data notice were driven solely by `config.security.injection_guard.external_source_tools` (default `fetch_url`/`web_fetch`). Added a tiny runtime registry `src/security/external-tools.ts` (`declareExternalSourceTool` / `effectiveExternalSourceTools`) consulted by BOTH injection-guard (before_agent_start notice) and pi-runner (tool_execution_end scanning). The capability registry marks every capability tool of a NON-builtin active provider as external (`providerId !== 'builtin'`) — builtin's hot-memory tool stays local/trusted. This holds the "a tool is a tool" trust boundary across the recall surface. Covered by `tests/memory-plug/external-content.test.ts`.

### dreem capability tools get functional execute handlers + tree/deep-search (dreem S5) — 2026-08-04 (Request: memory-system-redesign)

Post-evaluation gap: dreem's declared capability tools (`dream`/`hot-retrieval`/`graph`/`health`) had no `execute` handlers, so they registered as "[memory::dreem::X] declared without a handler" stubs, and the spec-listed `tree`/deeper-search were absent. Each capability now carries an `execute` that calls the configured backend (`/dream`, `/search` hot, `/graph`, `/health`, `/tree`, `/search` deep) via the provider's `request` helper and degrades gracefully (logged, surfaced `{error}`) at the provider level when the backend is unreachable. Added the `tree` and `deep-search` declarations. Reeboot's view system was not wired for these because all six dreem capabilities are tool-schema (data-query) surfaces, not interactive/non-tool-schema surfaces. Covered by `tests/memory-plug/dreem-capabilities.test.ts`.
