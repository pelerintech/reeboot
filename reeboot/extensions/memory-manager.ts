...
export function makeMemoryExtension(
  ...
  // Resolve config from pi — typed loosely to avoid requiring a full Config import
  ...
  pi.registerTool({
    ...
    execute: async (_id, params) => {
      ...
      if (!db) {
        return { content: [{ type: 'text' as const, text: JSON.stringify({ results: [], error: 'Database not available' }) }] };
      }
...
export default function memoryManagerExtension(pi: ExtensionAPI): void {
