...
describe('knowledge integration — loader wiring', () => {
  it('getBundledFactories includes knowledge-manager when knowledge.enabled=true', async () => {
    ...
    const factories = getBundledFactories({ id: 'test', workspacePath: join(tmpdir(), 'reeboot-ki') } as any, config as any);
    ...
    const factoriesDisabled = getBundledFactories({ id: 'test', workspacePath: join(tmpdir(), 'reeboot-ki') } as any, configDisabled as any);
