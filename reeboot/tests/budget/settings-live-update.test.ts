...
describe('TB-5-C: PUT /api/settings/budget updates live BudgetGuard', () => {
  ...
  beforeEach(async () => {
    tmpDir = mkdtempSync(join(tmpdir(), 'settings-live-test-'));
    db = makeDb();
    vi.resetModules();

    // Write a config with NO budget limits initially
  ...
  it('BudgetGuard uses updated config after PUT — verified via orchestrator updateBudgetConfig', async () => {
    // This test verifies the orchestrator exposes updateBudgetConfig() and
    // the server calls it on PUT.
    // We test by importing the Orchestrator class and checking the method exists.
    const { Orchestrator } = await import('@src/orchestrator.js');
    expect(typeof Orchestrator.prototype.updateBudgetConfig).toBe('function');
  });
});
