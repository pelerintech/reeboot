...
import { loadVecExtension, runKnowledgeMigration } from '../db/index.js';
...
export function makeKnowledgeExtension(
  ...
  pi.registerTool({
    ...
    execute: async (_id: string, params: { query: string; limit?: number }) => {
      if (!db) {
        return { content: [{ type: 'text' as const, text: 'Database not available.' }], details: {} };
      }
      ...
      const formatted = results.map((r: { source_tier: string; confidence: string; filename: string; content: string }, i: number) => {
